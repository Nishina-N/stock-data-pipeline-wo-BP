...
**系統ごとに独立**している（実測）。3ジョブを並列に走らせても 429 は出なかった。

| 系統 | 安全な間隔 |
|---|---|
| `/equities/master` | 8秒（6秒でも5回目に 429 の実績あり） |
| `/equities/bars/daily`・`/fins/*`・`/markets/*` | 3秒 |

`_jq_client.Client` は `min_interval` で間隔を強制し、429 は指数バックオフ（最大120秒）で待つ。
