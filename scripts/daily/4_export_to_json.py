...
※ RRS・indicators・raw 値の出力は廃止。
  ...
  - scores は sector/industry RS のみ（percentile）
  - individual RS は core の rs_percentile と重複するため scores へは出力しない
  - indicators 系統は出力しない（OHLCV から利用側で再計算する方針）

入力:
  - temp_prices.json
  - temp_rs_individual.json, temp_rs_sector.json, temp_rs_industry.json
    （individual は core への埋め込み・メタデータ生成に使用）
...
def main():
    ...
    # Individual RS は core の rs_percentile に埋め込み済み（重複のため scores へは出力しない）
