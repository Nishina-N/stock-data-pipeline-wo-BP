...
DATASETS = {
    ...
    # 現存銘柄の日次4本値。台帳 §4（C ランク）の照合用
    # （負値セル7銘柄・分割検出813件を独立情報源で裏取り）。
    # 🔴 bulk では取らないこと。bulk は Adj* 15列を落としており
    #    API 44列に対し 29列しか返らない（行数は一致するので気づけない）。
    #    delisted_bars と列構成を揃えるため日次 API で取る
    'bars_daily':    ('/equities/bars/daily', 'date', '2008-05-07', 'date'),
