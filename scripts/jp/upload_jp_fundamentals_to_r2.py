"""
upload_jp_fundamentals_to_r2.py

data/temp_fundamentals_jp.json を年別構成でR2にアップロード。
jp/stocks/fundamentals/quarterly/{year}/{code}.json

US版 scripts/monthly/upload_fundamentals_to_r2.py と同じ year-freeze 方針:
...
R2_PREFIX = "jp/stocks/fundamentals/quarterly/"
...
def upload_jp_fundamentals_to_r2(force_past=False):
    ...
    with open(TEMP_FUNDAMENTALS_JSON, encoding='utf-8') as f:
    ...
    for symbol, year_data in symbol_year_data.items():
        for year, quarters in year_data.items():
            key = f"{R2_PREFIX}{year}/{symbol}.json"
            if not force_past and year < CURRENT_YEAR and key in existing_keys:
                frozen_count += 1
                continue
            output = {'ticker': symbol, 'data': quarters, 'lastUpdated': now_iso}
            body_bytes = json.dumps(output, ensure_ascii=False).encode('utf-8')
    ...
    logging.info(f"Total files to upload: {total_files} (workers={MAX_WORKERS})")

    success_count = fail_count = 0
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {
            executor.submit(upload_single_object, endpoint, access_key, secret_key, bucket_name, key, body): key
            for key, body in upload_tasks
        }
    ...
    logging.info(f"Structure: {R2_PREFIX}{{year}}/{{code}}.json")
    return True
...
def main():
    ...
    try:
        return upload_jp_fundamentals_to_r2(force_past=args.force_past)
    except Exception as e:
