...
| `JGB2Y` | 日本国債2年物利回り(%) | 財務省公式CSV（`mof_jgbcm`） | 1974-09 | JP短期金利レジーム |
| `JGB10Y` | 日本国債10年物利回り(%) | 財務省公式CSV（`mof_jgbcm`） | 1986-07 | JP長期金利レジーム |

- `JGB2Y`/`JGB10Y` は Yahoo に存在しないため財務省「国債金利情報」CSV
  (`jgbcm_all.csv`、和暦・昭和49年(1974)〜)から`fetch_jp_macro_jgb.py`で同時取得。
  10年物の実測開始は1986-07（それ以前は10年物ベンチマーク自体が無い）。2年物は1974-09〜フル。
  利回り(%)を疑似OHLCVとして`open=high=low=close=利回り`、`volume=null` で格納
  （他シリーズと形式を揃えるため）。曲線傾き（10y−2y）は他シリーズ同様に保存せず利用側で計算
