"""
upload_individual_rrs_to_r2.py

Individual RRS JSONファイルをR2にアップロード（並列処理版）
入力: data/maintenance/r2/scores/RRS_scores/individual/{year}.json
出力: R2 bucket: scores/RRS_scores/individual/{year}.json
"""
import os
import sys
import boto3
...
LOCAL_DIR = os.path.join(R2_OUTPUT, "scores", "RRS_scores", "individual")
...
def main():
    """メイン処理"""
    logging.info("="*60)
    logging.info("UPLOAD INDIVIDUAL RRS TO R2 (PARALLEL)")
    logging.info("="*60)
    ...
    if not os.path.exists(LOCAL_DIR):
        ...
        logging.error("Please run 4_2_export_individual_rrs.py first")
        return False
    
    # R2にアップロード（並列処理）
    upload_to_r2_parallel(LOCAL_DIR, "scores/RRS_scores/individual", max_workers=MAX_WORKERS)
    ...
    logging.info(f"   rm data/maintenance/temp_rrs_*.pkl")
